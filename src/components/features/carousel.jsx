...
function Carousel ( props ) {
    ...
    useEffect( () => {
        if ( carouselRef.current && carouselRef.current.props.className.indexOf( 'product-single-carousel' ) > -1 ) {
            ...
            if ( parent.querySelector( "#carousel-quick-dots .owl-dot" ) ) {
                if ( parent.querySelector( "#carousel-quick-dots .owl-dot.active" ) ) {
                    parent.querySelector( "#carousel-quick-dots .owl-dot.active" ).classList.remove( "active" );
                }

                parent.querySelector( "#carousel-quick-dots .owl-dot" ).classList.add( "active" );
                carousel.goTo( 0 );
                dots = parent.querySelectorAll( '#carousel-quick-dots .owl-dot' );
    ...
    const init = () => {
        ...
        if ( carouselRef.current && carouselRef.current.props.className.indexOf( 'home-slider' ) > -1 && document.querySelector( ".home-slider-thumbs" ) ) {
    ...
    function getOption ( settings, addClass = '' ) {
        ...
        if ( addClass.indexOf( 'post-slider' ) > -1 ) {
            options = extraSetting2;
        }
        if ( addClass.indexOf( 'product-single-carousel' ) > -1 ) {
            options = Object.assign( {}, extraSetting4, options );
        }
